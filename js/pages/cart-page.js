...
function render() {
  auditCartStock();
  const cart = Cart.get();
  const sub  = Cart.subtotal(PRODUCTS);
  ...
  document.getElementById('cart-items-list')?.addEventListener('click', e => {
    ...
    if (btn.dataset.action === 'dec-qty') Cart.changeQty(id, -1, p?.stock ?? 99);
...
// ── INIT ─────────────────────────────────────────────────
initHeader({
  onSearchSubmit(q) { if (q.trim()) window.location.href = `search.html?q=${encodeURIComponent(q.trim())}`; },
});

Cart.onChange(() => { updateBadge(); render(); });
