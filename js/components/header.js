...
const NAV_GROUPS = [
  ...
  },
  {
    label: "About Us",
    href: "about.html",
    icon: `<svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>`,
  },
  {
    label: "FAQ",
    href: "faq.html",
    icon: `<svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3"/><circle cx="12" cy="17" r="0.5" fill="currentColor" stroke="currentColor" stroke-width="1"/></svg>`,
  },
  {
    label: "Install Guide",
...
function syncLangButtons(activeLang) {
  const active =
    activeLang ?? (document.cookie.includes("googtrans=/en/fr") ? "fr" : "en");
  document.querySelectorAll(".lang-switch-thumb").forEach((thumb) => {
    thumb.style.transform =
      active === "fr" ? "translateX(32px)" : "translateX(0)";
  });
  document.querySelectorAll(".lang-btn").forEach((btn) => {
    btn.style.color =
      btn.dataset.lang === active ? "#fff" : "rgba(255,255,255,0.5)";
  });
}
...
export function initHeader({
  ...
  window.addEventListener("scroll", () => {
    ...
    document
      ...
      ?.classList.toggle("banner-hidden", scrolled);
  });
  ...
  document
    ...
    ?.addEventListener("click", () => onCartOpen?.());

  // Mobile search button
  document
    .getElementById("mob-search-btn")
